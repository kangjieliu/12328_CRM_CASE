...
//         myToolBar.set_list("SelectProcess", m_dyProcess);
    ...
    private void Fillin_grid()
    {
         ...
         if(dtStru.getCount()>0)
         {
             ...
             mydg = (my_dataGrid) mRet.oRet;
             mydg.fill_header("dgvList", "CASEID", m_strFilter() + m_strOrder_by());
         }
    }
    ...
    private String m_strFilter()
    {
           ...
           String strFilter = m_strFilter_base();
            ...
            //add by xutt 20171023
            if (strFilter.length() > 0) strFilter = myString.Format("   {0} AND CASETYPE={1} ", strFilter, pType);
            else strFilter = myString.Format("   CASETYPE={0} ", pType);
