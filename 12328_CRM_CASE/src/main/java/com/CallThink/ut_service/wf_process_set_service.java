...
import com.ToneThink.ctsTools.WebUI.my_Field;
    ...
    public class wf_process_set_service extends UltraCRM_Page
    {
...
*/
        public void myToolBar_btnItemClick(Object sender, String name, String parms, int nparms)//, String parms, int nparms)
        {
            ...
            else if (name.equals("Submit"))//提交 
            {
                 ...
                 {
                     //if (myCaseInfo.nStatus != 0 && myCaseInfo.nStatus != 98 && myCaseInfo.nStatus != 99) //只有这三种状态才可以提交下一环节
                ...
                int nProcess_next = myWFCase.get_process_index(1, mRef);
                int nAccept_auto=mRef.oRet;
                //检查是否有不为空的字段
                ...
                if (nResult == 1)
                {
                  ...
                  //服务管理定制，提交成功后，如果当前座席有调度权限，则判断是否进入调度页面
                    ...
                    Change_Ghid_rel(strGhid, htCase);                        
      ...
      /// <summary>
        ...
        private boolean Check_task_finished()
        {
            ...
            if (rc >= 0)
            {
                if (dtTable.Rows().Count() == 0) //没有任务可以直接提交
                    bResult = true;
                else
                {
      ...
      //add by gaoww 20151103 增加ghid_rel各个环节有关联的工号处理
        ...
        /// 变更工单的GHID_REL
        /// </summary>
        /// <param name="htCase">要变化的工单内容</param>
        /// <param name="strGhid">当前关联的工号</param>
        private void Change_Ghid_rel(String strGhid,HashMap  htCase)
        {
            if (strGhid != "")
            {
                //strGhid = strGhid.TrimEnd('-'); //临时封上
                if (strGhid != Functions.ht_Get_strValue("GHID", myCaseInfo.htCase)) //如果不是工单创建人，则检查是否在关联工号里
                {
                    String strGhid_rel_scr = Functions.ht_Get_strValue("GHID_REL", myCaseInfo.htCase);
                    List alGhid_rel = Arrays.asList(strGhid_rel_scr.split(","));
                    if (alGhid_rel.contains(strGhid) == false)
                    {
                        if (strGhid_rel_scr != "") strGhid_rel_scr += ",";
                        strGhid_rel_scr += strGhid;                        
            ...
            {
                int rc = myCase.UpdateCaseRecord(pCaseId, htCase);
            }
        }
    }
