...
public class ut_case_ctrl extends BaseController {
	...
	public String case_business_left_tree(String strNodeId, Model model) {
		return "ut_case/case_business_left_tree";
	}
	...
	// -打印树形结构
	@RequestMapping(value = "ut_case/case_business_left_tree_print.aspx")
	public void getcase_business_left_tree_Data(Integer kType, String strNodeId, PrintWriter out) {
		String strHtml = "";
		String strHolder = "tvList";
		case_business_left_tree myForm = new case_business_left_tree(0);
		if (myString.IsEmpty(strNodeId))
			strHtml = myForm.getTreeView_root(strHolder);
		else
			strHtml = myForm.getTreeView_child(strNodeId);
	...
	public String ProcessRequest(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (i > 0) {
				...
				if (str2 == "agent_login") {
					this.pmAgent = new pmAgent_info();
				} else if (str2 == "check_agent_permit") {
					this.pmAgent = new pmAgent_info();
				} else {
					this.pmAgent = fun_main.GetParm();
				}
				pmRet localpmRet = case_data.Prosess_data(str2, localHashMap);
